...
export async function PATCH(req: NextRequest) {
     ...
     try {
          ...
          const result = await response.json();
            console.log({ url });
    console.log("Agent Result Response:", response);
    console.log("Agent Result Body:", result);
    console.log("Payload Sent:", updateAgentPayload);
    if (!response.ok) {
     throw new Error(
       `Backend API error: ${result.message || result.error || "Unknown error"}`,
     );
   }
   return NextResponse.json(result);
 } catch (error: any) {
