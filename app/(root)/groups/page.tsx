...
export async function generateMetadata({
    params}:PageProps) {
    return {
        title: `Transpay - Groups ${(await params).id}`
    }
}
export default async function GroupVehiclePage({
    searchParams
	...
	return (
		...
		<div className="items-center justify-between flex font-bold py-5 uppercase">
	</div>
	
    {session.user.role.toLowerCase() !== "agent" && session.user.role.toLowerCase() !== "admin" && session.user.role.toLowerCase() !== "green_engine" ? (
        <>
        <div className="mb-10 flex flex-col gap-5">
            <DataTable showSearch searchWith='groupName' searchWithPlaceholder='Enter company/business name' showColumns columns={groupedVehiclesColumns} data={groups.vehicle_groups}/>
        </div>
        {groups && (
            <PaginationISCE
