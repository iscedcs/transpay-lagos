...
export default async function Agents({ searchParams }: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
     const resolvedSearchParams = await searchParams;
     const page = resolvedSearchParams["page"] ?? "1";
     const limit = resolvedSearchParams["limit"] ?? "15";
     const agents = await getUsers({ limit: Number(limit), role: "LGA_AGENT" });
               ...
               <div className="flex items-center justify-between font-bold uppercase">
                    ...
                    <div className="shrink-0 grow-0">
                         <Button className="justify-start rounded-xl bg-primary-800 text-white" asChild variant={"default"}>
                              <Link href={"/agents/new-agent"} className="shrink-0 whitespace-nowrap">
               ...
               <div className="flex flex-col gap-5">
                    <Tabs defaultValue="all" className="w-full">
                         ...
                         <TabsContent value="all">
                              <DataTable showSearch searchWith="name" searchWithPlaceholder="Search with name" 
                              showColumns 
