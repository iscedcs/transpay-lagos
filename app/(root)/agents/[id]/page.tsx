...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
    ...
    return {
        title: `Transpay Agent - ${agent?.firstName?.toLocaleUpperCase() || ""} ${agent?.lastName?.toLocaleUpperCase() || ""}`.trim() || "Transpay Agent",
    };
}

export default async function SingularAgent({ params }: { params: Promise<{ id: string }> }) {
    const resolvedParams = await params;
    const agent = await getUser(resolvedParams.id);
    ...
    return (
        <div className="flex h-full w-full flex-col gap-5 p-3 xs:p-5">
            <div className="flex items-center justify-between">
                <div className="shrink-0 grow-0">{agent?.firstName?.toLocaleUpperCase() || ""} {agent?.lastName?.toLocaleUpperCase() || ""}</div>
            </div>
