...
export async function getPaymentTotalsForStickers({
  ...
  const baseWhereClause: any = {};
  if (revenueType) {
    baseWhereClause.revenueCode = { startsWith: REVENUE_TYPES.ISCE };
  }
...
export async function getChatData(frequency: 'all' | 'yearly' | 'monthly' | 'weekly' | 'daily', date: Date, revenueType: RevenueType = 'CVOF') {
  let startDate: Date
  let endDate: Date
  let groupBy: string
  ...
  const transactions = await db.paymentNotification.findMany({
    where: {
      ...
      revenueCode: REVENUE_TYPES[revenueType],
