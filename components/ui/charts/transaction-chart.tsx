...
export function AllTransactionChart() {
     ...
     const allTimeHistory = eachYearOfInterval({ start: new Date("2024-11-01"), end: new Date() });
     ...
     function CustomTooltip({ active, payload, label }: { payload: any; label?: any; active?: boolean }) {
     ...
     return (
          <ScrollArea>
               <BarChart width={900} height={300} data={allTotals}>
                    ...
                    <Tooltip content={<CustomTooltip payload={allTotals.map((item) => item.disp)} />} />
          ...
          const fetchAllTotal = async () => {
               const totals = await Promise.all(
                    dayHistory.map(async (time) => {
                         ...
                         const total = await getPaymentTotalsOld(startOfHour(time), endOfHour(time));
                         return {
                              name: format(time, "kk"),
     ...
     return (
          <div className="">
               ...
               <BarChart width={800} height={300} data={dayTotal}>
                    <XAxis dataKey="name" stroke="#8884d8" />
                    <YAxis tickFormatter={formatCurrency} />
                    <Tooltip content={<CustomTooltip payload={dayTotal.map((item) => item.disp)} />} />
                    <Legend width={100} wrapperStyle={{ top: 40, right: 20, backgroundColor: "#f5f5f5", border: "1px solid #d5d5d5", borderRadius: 3, lineHeight: "40px" }} />
                    <CartesianGrid stroke="#f3efd0" strokeDasharray="5 5" />
                    <Bar dataKey="total" fill="#4d4100" barSize={30} />
               </BarChart>
...
export function WeeklyTransactionChart({ week, month, year }: { week: string; month: string; year: string }) {
     const [isMounted, setIsMounted] = useState(false);
     ...
     const selectedDate = useMemo(() => {
          switch (week) {
               case "week1":
                    return eachDayOfInterval({ start: new Date(Number(year), monthNumber - 1, 1), end: endOfWeek(new Date(Number(year), monthNumber - 1, 1)) });
               case "week2":
                    return eachDayOfInterval({ start: new Date(Number(year), monthNumber - 1, 8), end: endOfWeek(new Date(Number(year), monthNumber - 1, 8)) });
               case "week3":
                    return eachDayOfInterval({ start: new Date(Number(year), monthNumber - 1, 15), end: endOfWeek(new Date(Number(year), monthNumber - 1, 15)) });
               case "week4":
                    return eachDayOfInterval({ start: new Date(Number(year), monthNumber - 1, 22), end: endOfWeek(new Date(Number(year), monthNumber - 1, 22)) });
               default:
                    return [];
          }
          ...
          const fetchAllTotal = async () => {
               const totals = await Promise.all(
                    selectedDate.map(async (week) => {
                         const total = await getPaymentTotalsOld(startOfDay(week), endOfDay(week));
                         return {
     ...
     return (
          <div className="">
               ...
               <BarChart width={800} height={300} data={monthTotal}>
                    <XAxis dataKey="name" stroke="#8884d8" />
                    <YAxis tickFormatter={formatCurrency} />
                    <Tooltip content={<CustomTooltip payload={monthTotal.map((item) => item.disp)} />} />
                    <Legend width={100} wrapperStyle={{ top: 40, right: 20, backgroundColor: "#f5f5f5", border: "1px solid #d5d5d5", borderRadius: 3, lineHeight: "40px" }} />
                    <CartesianGrid stroke="#f3efd0" strokeDasharray="5 5" />
                    <Bar dataKey="total" fill="#4d4100" barSize={30} />
               </BarChart>
...
export function MonthlyTransactionChart({ month, year }: { month: string; year: string }) {
     const [isMounted, setIsMounted] = useState(false);
     const [monthTotal, setMonthTotal] = useState<ChartDataPoint[]>([]);
     const months = ["january", "february", "march", "april", "may", "june", "july", "august", "september", "october", "november", "december"];
          ...
          const fetchAllTotal = async () => {
               const totals = await Promise.all(
                    monthHistory.map(async (month) => {
                         const total = await getPaymentTotalsOld(startOfWeek(month), endOfWeek(month));
                         return {
                              name: `${format(startOfWeek(month), "MM/dd")} - ${format(endOfWeek(month), "MM/dd")}`,
                              disp: formatCurrency(Math.round(Number(total.customRangeTotal))),
                              total: Math.ceil(Number(total.customRangeTotal)),
                         };
                    }),
...
export function YearlyTransactionChart() {
     ...
     const yearHistory = eachMonthOfInterval({ start: new Date("2024-08-01"), end: new Date() });
          ...
          const fetchAllTotal = async () => {
               const totals = await Promise.all(
                    yearHistory.map(async (year) => {
                         const total = await getPaymentTotalsOld(startOfMonth(year), endOfMonth(year));
                         return {
     ...
     return (
          <div className="">
               <BarChart width={800} height={300} data={yearTotal}>
                    <XAxis dataKey="name" stroke="#8884d8" />
                    <YAxis tickFormatter={formatCurrency} />
                    <Tooltip content={<CustomTooltip payload={yearTotal.map((item) => item.disp)} />} />
                    <Legend width={100} wrapperStyle={{ top: 40, right: 20, backgroundColor: "#f5f5f5", border: "1px solid #d5d5d5", borderRadius: 3, lineHeight: "40px" }} />
                    <CartesianGrid stroke="#f3efd0" strokeDasharray="5 5" />
                    <Bar dataKey="total" fill="#4d4100" barSize={30} />
               </BarChart>
