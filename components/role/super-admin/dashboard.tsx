import { getPaymentTotals, getPaymentTotalsForStickers } from "@/actions/payment-notification";
...
export default async function DashboardSuperAdmin() {
     ...
     const pieChartData = Object.keys(categoryCounts.categories).map((category) => ({
          name: category,
          value: categoryCounts.categories[category],
     }));

     const redenderRevenueCards = (data: any, revenueType: 'CVOF' | 'ISCE' | 'FAREFLEX') => (
          <div className="grid grid-cols-1 gap-5 xs:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
               <Suspense fallback={<Skeleton className="flex h-24 w-full flex-col justify-between rounded-2xl bg-secondary p-3 shadow-md"/>}>
                    <RevenueAmountCardNew link={`/dashboard`} type="TOTAL" title="All Time Revenue" desc="All time" total={Number(data.allTimeTotal)}/>
                    <RevenueAmountCardNew link={`/dashboard/history/yearly?revenue=${revenueType}`} type="YEAR" title="Year Till Date Revenue" desc="Year till date" total={Number(data.yearToDateTotal)}/>
                    <RevenueAmountCardNew link={`/dashboard/history/monthly?revenue=${revenueType}`} type="MONTH" title="Month Till Date Revenue" desc="Month till date" total={Number(data.monthToDateTotal)}/>
                    <RevenueAmountCardNew link={`/dashboard/history/weekly?revenue=${revenueType}`} type="WEEK" title="Week Till Date Revenue"  desc="Week till date" total={Number(data.weekToDateTotal)}/>
                    <RevenueAmountCardNew link={`/dashboard/history/daily?revenue=${revenueType}`} type="DAY" title="Day Till Date Revenue" desc="Day till date" total={Number(data.dayToDateTotal)}/>
               </Suspense>
     ...
     return (
          <div className="w-full">
               ...
               <div className="grid w-full gap-5 xl:grid-cols-2">
                    ...
                    <VehicleTypesProgress data={categoryCounts} />
               </div>
          </div>
     );
}
