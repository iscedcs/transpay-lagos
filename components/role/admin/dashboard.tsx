...
export default async function DashboardAdmin() {
     ...
     const allAgents = await getUsers({ role: "LGA_AGENT" });
     ...
     return (
          <div>
                         ...
                         <Card>
                              ...
                              <CardContent className="grid grid-cols-1 px-2 py-4">
                                   <div className="pointer-events-none relative grid gap-2 rounded-md border border-primary bg-secondary p-2">
                                        ...
                                        <p className="text-2xl text-background">{allAgents?.meta?.total ?? 0}</p>
                                   </div>
                              </CardContent>
                         </Card>
                    </Link>
                    <Link href={"/dashboard/my-agents?page=1&limit=15"}>
                         <Card>
                              ...
                              <CardContent className="grid grid-cols-1 px-2 py-4">
                                   <div className="pointer-events-none relative grid gap-2 rounded-md border border-primary bg-secondary p-2">
                                        <p className="font-bold leading-none text-primary">Total</p>
                                        <p className="text-2xl text-background">{myAgents?.totalAgents ?? 0}</p>
                                   </div>
                              </CardContent>
                         </Card>
                    </Link>
                    <Link href={"/vehicles"}>
                         <Card>
                              ...
                              <CardContent className="grid grid-cols-1 px-2 py-4">
                                   <div className="pointer-events-none relative grid gap-2 rounded-md border border-primary bg-secondary p-2">
                                        <p className="font-bold leading-none text-primary">Total</p>
                                        <p className="text-2xl text-background">{allVehicles?.rows?.length || 0}</p>
                                    </div>
                              </CardContent>
                         </Card>
                    </Link>
               ...
               <div className="mb-20 flex flex-col gap-2">
                    <DataTable showSearch searchWith="name" searchWithPlaceholder="Search with name" showColumns columns={agentsColumns} data={myAgents?.agents ?? []} />
